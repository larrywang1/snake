...
func ability():
	...
	if side in [1, 3]:
		for i in range(LevelManager.level.grid_size.x):
			...
			if attacked_space != null:
				if side == 3 and attacked_space.global_position.x > global_position.x:
					continue
				elif side == 1 and attacked_space.global_position.x < global_position.x:
					continue
	...
	if side in [0, 2]:
		for i in range(LevelManager.level.grid_size.y):
			...
			if attacked_space != null:
				if side == 0 and attacked_space.global_position.y > global_position.y:
					continue
				elif side == 2 and attacked_space.global_position.y < global_position.y:
					continue
				if !attacked_space.is_in_group("snake"):
					if attacked_space.has_method("hit") and attacked_space not in attacked_enemies:
...
func inspect():
	...
	match side:
		0:
			InspectManager.add_inspect(self, "BD-L-A", "Fires a piercing laser upwards", {"attack": damage, "health": health, "max_cd" : cooldown, "current_cd" : current_cooldown, "manual" : manual_string}, manual, activated, upgraded, unupgraded, attributes)
		1:
			InspectManager.add_inspect(self, "BD-L-A", "Fires a piercing laser rightwards", {"attack": damage, "health": health, "max_cd" : cooldown, "current_cd" : current_cooldown, "manual" : manual_string}, manual, activated, upgraded, unupgraded, attributes)
		2:
			InspectManager.add_inspect(self, "BD-L-A", "Fires a piercing laser downwards", {"attack": damage, "health": health, "max_cd" : cooldown, "current_cd" : current_cooldown, "manual" : manual_string}, manual, activated, upgraded, unupgraded, attributes)
		3:
			InspectManager.add_inspect(self, "BD-L-A", "Fires a piercing laser leftwards", {"attack": damage, "health": health, "max_cd" : cooldown, "current_cd" : current_cooldown, "manual" : manual_string}, manual, activated, upgraded, unupgraded, attributes)
