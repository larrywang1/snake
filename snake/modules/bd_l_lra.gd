...
func ability():
	...
	for i in range(LevelManager.level.grid_size.x):
		...
		if attacked_space != null:
			if left and attacked_space.global_position.x > global_position.x:
				continue
			elif !left and attacked_space.global_position.x < global_position.x:
				continue
...
func inspect():
	if manual:
		...
		manual_string = "X"
	InspectManager.add_inspect(self, "BD-L-LRA", "Fires an alternating piercing laser leftwards and rightwards", {"attack": damage, "health": health, "max_cd" : cooldown, "current_cd" : current_cooldown, "manual" : manual_string}, manual, activated, upgraded, unupgraded, attributes)
	var size = get_tree().get_first_node_in_group("level").grid_size.x
	var array = []
	for i in range(size):
		if i < int(global_position.x / 16) and left:
			array.append(Vector2( i, global_position.y / 16))
		elif i > int(global_position.x / 16) and !left:
			array.append(Vector2( i, global_position.y / 16))
